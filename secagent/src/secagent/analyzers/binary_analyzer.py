...
def _detect_arch(arch_str: str) -> tuple[int, int]:
    ...
    from capstone import CS_ARCH_X86, CS_ARCH_ARM, CS_ARCH_ARM64, CS_ARCH_MIPS
    from capstone import CS_MODE_32, CS_MODE_64, CS_MODE_ARM
...
def analyze_binary(file_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(binary, lief.PE.Binary):
            ...
            sec["nx"] = bool(dll_chars & int(lief.PE.DLL_CHARACTERISTICS.NX_COMPAT))
            sec["aslr"] = bool(dll_chars & int(lief.PE.DLL_CHARACTERISTICS.DYNAMIC_BASE))
            sec["cfg"] = bool(dll_chars & int(lief.PE.DLL_CHARACTERISTICS.GUARD_CF))
            sec["dep"] = sec["nx"]
            sec["authenticode"] = bool(binary.has_signatures)
        elif isinstance(binary, lief.ELF.Binary):
            sec["nx"] = not any(
                seg.type == lief.ELF.SEGMENT_TYPES.GNU_STACK
                and int(seg.flags) & int(lief.ELF.SEGMENT_FLAGS.X)
                for seg in binary.segments
            )
            sec["pie"] = binary.header.file_type == lief.ELF.E_TYPE.DYN
            sec["canary"] = any(
                s.name == "__stack_chk_fail" for s in binary.dynamic_symbols
            )
            sec["relro"] = any(
                seg.type == lief.ELF.SEGMENT_TYPES.GNU_RELRO for seg in binary.segments
            )
